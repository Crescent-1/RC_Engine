...
class NoveltyScorer:
    ...
    def score(self, fp: Fingerprint, blueprint_sims: dict[str, float],
        ...
        # Cheap mode: demote soft channels to corpus_flags so a paid fullset
        # is not discarded for stylometry/rhythm/curve alone.
        soft_prefixes = getattr(config, "SOFT_NOVELTY_BREACH_PREFIXES", ())
        hard_breached, soft_breached = [], []
        for b in breached:
            if any(b.startswith(p) for p in soft_prefixes):
                soft_breached.append(b)
            else:
                hard_breached.append(b)
        breached = hard_breached
        ...
        corpus_flags = corpus_flags + [f"soft_novelty: {b}" for b in soft_breached[:6]]
