"""Command-line interface — RC_ENGINE_DEV (cheap-default sandbox).

Run from the RC_ENGINE_DEV folder:

  python -m rc_engine.cli estimate
  python -m rc_engine.cli selftest
  python -m rc_engine.cli generate --elite 1 --hard 1          # cheap mode
  python -m rc_engine.cli generate --elite 1 --mode factory    # multi-stage
  python -m rc_engine.cli generate --hard 2 --db ../rc_pipeline.db  # live corpus

Does NOT modify production `rc_engine/`. Default DB: ./rc_engine_dev.db
"""
...
_PROJECT_ROOT = _DEV_ROOT.parent  # parent RC Gen Project


def _load_env() -> None:
    """Load parent project .env (override=True so valid key wins over stale env)."""
    for p in (_DEV_ROOT / ".env", _PROJECT_ROOT / ".env"):
        if not p.is_file():
            continue
        try:
...
STAGES_CHEAP = ["refine", "fullset", "solver", "judge"]
STAGES_FACTORY = ["refine", "render", "compliance", "questions", "solver", "judge"]
...
def cmd_estimate(_args) -> int:
    ok = True
    for label, stages in (("CHEAP (default)", STAGES_CHEAP),
                          ("FACTORY", STAGES_FACTORY)):
        print(f"\n=== {label} ===")
        print(f"{'tier':8s} {'stage':12s} {'model':28s} {'typical':>9s}")
        for tier in ("medium", "hard", "elite"):
            happy = 0.0
            for stage in stages:
                if stage not in config.STAGE_CONFIG:
                    continue
                if label.startswith("CHEAP"):
                    if stage == "solver" and not config.CHEAP_RUN_SOLVER:
                        continue
                    if stage == "judge" and not config.CHEAP_RUN_JUDGE:
                        continue
            ...
            print(f"{tier:8s} TOTAL        happy path                 {happy:>9.4f}  "
                  f"(cap ${budget:.2f})")
            if happy > budget:
                print("  !! happy path exceeds cap")
                ok = False
            print()
...
def cmd_selftest(_args) -> int:
    ...
    print("[4/5] Mock cheap pipeline (3 RCs, $0, temp DB)...")
    fd, tmpdb = tempfile.mkstemp(suffix=".db")
    os.close(fd)
    os.unlink(tmpdb)
    try:
        history = HistoryStore(tmpdb)
        pipe = RCPipeline(history, MockLLMClient(), embed=False, mode="cheap")
        ...
        if len(shipped) != 3:
            failures.append(f"cheap: expected 3 shipped RCs, got {len(shipped)} "
    ...
    print("[5/5] Mock factory path (1 elite, $0, temp DB)...")
    fd, tmpdb = tempfile.mkstemp(suffix=".db")
    os.close(fd)
    os.unlink(tmpdb)
    try:
        history = HistoryStore(tmpdb)
        pipe = RCPipeline(history, MockLLMClient(), embed=False, mode="factory")
        res = pipe.generate_one("elite")
        if res.rc_id is None or res.status not in ("approved", "needs_review",
                                                     "solver_dispute"):
            failures.append(f"factory: expected shipped RC, got {res.status} "
                            f"notes={res.notes}")
        else:
            print(f"      OK: factory shipped {res.rc_id} ({res.status})")
        history.close()
    finally:
        if os.path.exists(tmpdb):
            os.unlink(tmpdb)
    ...
    if failures:
        ...
        for f in failures:
            print(f"  - {f}")
        return 1
    print("SELFTEST PASSED — DEV cheap + factory paths OK; cost guard active.")
    return 0
...
def cmd_generate(args) -> int:
    ...
    mode = getattr(args, "mode", None) or config.DEFAULT_MODE
    if args.dry_run:
        llm = MockLLMClient()
        if args.db == config.DB_PATH:
            args.db = str(_DEV_ROOT / "rc_engine_dev_dryrun.db")
        print(f"[dry-run] MockLLMClient $0 | mode={mode} | DB={args.db}")
    else:
        if not os.environ.get("ANTHROPIC_API_KEY"):
            print("ANTHROPIC_API_KEY not set (load .env from project root).")
            return 1
        llm = LLMClient()

    history = HistoryStore(args.db)
    corpus = None
    corpus_path = getattr(config, "CORPUS_DB_PATH", None)
    if (corpus_path and os.path.isfile(corpus_path)
            and os.path.abspath(corpus_path) != os.path.abspath(args.db)):
        try:
            corpus = HistoryStore(corpus_path)
            print(f"[corpus] novelty window <- {corpus_path}")
        except Exception as e:
            print(f"[corpus] unavailable ({e}) — using write DB only")
    embed = not (args.no_embed or args.dry_run)
    pipe = RCPipeline(history, llm, embed=embed, mode=mode, corpus_history=corpus)
    ...
    if not args.no_export:
        out_dir = str(_DEV_ROOT / ("exported_dryrun" if args.dry_run else "exported_rc_sets"))
...
def main(argv=None) -> int:
    _load_env()
    # Prefer running with cwd = RC_ENGINE_DEV for relative exports
    try:
        os.chdir(_DEV_ROOT)
    except OSError:
        pass
    p = argparse.ArgumentParser(prog="rc_engine_dev", description=__doc__,
    ...
    g = sub.add_parser("generate", help="generate RCs (default mode=cheap)")
    g.add_argument("--medium", type=int, default=0)
    g.add_argument("--hard", type=int, default=0)
    g.add_argument("--elite", type=int, default=0)
    g.add_argument("--mode", choices=["cheap", "factory"], default=config.DEFAULT_MODE,
                   help="cheap=one fullset call (default); factory=multi-stage")
    g.add_argument("--db", default=config.DB_PATH,
                   help="SQLite path (default: RC_ENGINE_DEV/rc_engine_dev.db)")
