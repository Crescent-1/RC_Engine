...
class MockLLMClient:
    ...
    def _fullset(self, ctx) -> str:
        """Cheap-mode mock: passage + questions JSON in one payload."""
        passage = self._render(ctx)
        qjson = self._questions(ctx)
        return f"[PASSAGE]\n{passage}\n\n[QUESTIONS_JSON]\n{qjson}\n"

    def _length_fix(self, ctx) -> str:
        """Same JSON shape as questions (passage kept by pipeline)."""
        return self._questions(ctx)
